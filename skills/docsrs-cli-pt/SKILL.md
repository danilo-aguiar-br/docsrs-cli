---
name: docsrs-cli
description: Esta skill DEVE ativar para docsrs-cli, busca no crates.io, fetch no docs.rs, lookup de item rustdoc, get-item, readme, search-crates, search-in-crate, version, doctor, commands, schema, completions, cache, config, envelopes JSON de agente, dry-run e cache ou config XDG. Ela DEVE ensinar o catálogo completo de comandos, argv exato, contratos JSON no stdout, exit codes, retry, allowlist de hosts e ciclo one-shot BORN-EXECUTE-FINALIZE-DIE para o agente obter docs de crates Rust sem raspar HTML.
---
