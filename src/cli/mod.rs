...
//!
//! [`overrides`] applies the parsed flags onto the loaded configuration.
//! [`agent_args`] holds the payload reduction flags shared by every subcommand.
...
pub use agent_args::AgentArgs;

use crate::config::{DEFAULT_PER_PAGE, DEFAULT_SEARCH_LIMIT};
...
pub struct Cli {
    ...
    /// Agent-native payload reduction knobs (see [`AgentArgs`]).
    #[command(flatten)]
    pub agent: AgentArgs,
...
pub enum Commands {
    ...
    GetItem {
        /// Crate name on crates.io or a stdlib crate (`name` or `name@1.2.3`).
        crate_name: String,
        /// Kind: module|mod|struct|trait|enum|union|fn|function|method|type|const|constant|static|macro|attr|attribute|derive|variant|structfield|field
        item_type: String,
        /// Path with `::` or `/` (e.g. `Runtime::new`, `runtime/Runtime`, `tokio::spawn`)
        ///
        /// `method`, `type` and `const` also accept `Parent::member` for members
        /// that rustdoc renders as an anchor on the parent page instead of a page
        /// of their own (`iter::Iterator::Item`, `time::Duration::MAX`).
        item_path: String,
        /// Optional crate version (`latest` when omitted). Conflicts if it differs from `crate@…`.
        #[arg(long)]
        crate_version: Option<String>,
        /// On 404, always report the suggestion list, even when it is empty ("(none)").
        ///
        /// Recovery already ranks nearby symbols from all.html without this flag
        /// (exact/prefix/substring/edit-distance); the flag only guarantees the
        /// field is present so an agent can tell "no candidates" from "not tried".
        #[arg(long)]
        suggest: bool,
    },
...
pub enum CacheAction {
    ...
    ///
    /// Destructive verbs designate their target in argv and never inherit it
    /// from ambient state. Without `--cache-dir` the root comes from the XDG
    /// layer, which the caller never named, so `--yes` is the explicit waiver
    /// that accepts an ambient target. Absent both, this fails with exit 64 and
    /// deletes nothing.
    Clear {
        /// Accept the ambient (XDG-resolved) cache root as the deletion target
        #[arg(long)]
        yes: bool,
    },
...
pub enum ConfigAction {
    ...
    Path,
    /// Print effective runtime configuration (merged defaults + TOML + CLI; no env)
    Show,
    /// Create a default config.toml under the resolved config directory
    ///
    /// `--force` replaces a file the caller may never have named. Destructive
    /// verbs designate their target in argv and never inherit it from ambient
    /// state, so `--config-dir` names the directory and `--yes` is the explicit
    /// waiver that accepts the XDG one. With `--force` and neither of the two,
    /// this fails with exit 64 and writes nothing. Without `--force` nothing is
    /// destroyed, so no waiver is asked for.
    Init {
        /// Overwrite an existing config.toml
        #[arg(long)]
        force: bool,
        /// Accept the ambient (XDG-resolved) config directory as the target of `--force`
        #[arg(long)]
        yes: bool,
    },
}
...
impl Cli {
    ...
    pub fn validate_format_conflict(&self) -> Result<(), crate::error::AppError> {
        ...
        {
            return Err(AppError::of(ErrorDetail::JsonFormatConflict));
        }
        Ok(())
    }
}
